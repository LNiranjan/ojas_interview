class TvShowsController < ApplicationController
  ...
  def Search
    tv_show1 = TvShow.find_by_show(params[:search])
    tv_show2 = TvShow.find_by_channel(params[:search])
    return @tv_show = tv_show1 if tv_show1.blank?
    return @tv_show = tv_show2 if tv_show2.blank?
  end

  def favorite
    @tv_show = TvShow.find_by_show(params[:show])
  ...
  private
    ...
    def tv_show_params
      params.require(:tv_show).permit(:channel, :show, :timing)
    end
end
